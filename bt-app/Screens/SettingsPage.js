...
const SettingsPage = () => {
  ...
  useEffect(() => {
    ...
    const autoConnectDevices = async () => {
      // Try to connect to scale if we have an address saved
      if (scaleAddress) {
        try {
          await connectToDevice(scaleAddress);
          console.log("Auto-connected to scale:", scaleAddress);
        } catch (error) {
          console.error("Failed to auto-connect to scale:", error);
        }
      }
      
      // Try to connect to printer if we have an address saved
      if (printerAddress) {
        try {
          await connectToDevice(printerAddress);
          console.log("Auto-connected to printer:", printerAddress);
        } catch (error) {
          console.error("Failed to auto-connect to printer:", error);
        }
      }
    };
    
    // Start auto-connection process after a short delay to ensure Bluetooth is initialized
    setTimeout(autoConnectDevices, 1000);
  }, []);
